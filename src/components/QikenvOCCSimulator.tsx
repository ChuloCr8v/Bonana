...
import { Terminal, RefreshCw, AlertCircle, CheckCircle2, GitBranch, ArrowRight } from 'lucide-react';
...
export const QikenvOCCSimulator: React.FC = () => {
  ...
  const [outputLog, setOutputLog] = useState<Array<{ text: string; type?: 'info' | 'error' | 'success' }>>([
  ...
  const simulateTeammatePush = () => {
    ...
    setOutputLog((prev) => [
      ...prev,
      { text: '--- EVENT: Teammate pushed commit [c9b312] to remote ---', type: 'info' },
      { text: 'Remote updated: v1.3.0 [c9b312] (Added REDIS_CACHE_TTL)', type: 'info' },
    ...
    setTimeout(() => {
      if (cmd === 'status') {
        ...
        setOutputLog((prev) => [
          ...prev,
          { text: '$ qikenv status', type: 'info' },
          { text: `Local State:  v1.${localVersion}.0 [${localHash}]`, type: 'info' },
          { text: `Remote State: v1.${remoteVersion}.0 [${remoteHash}]`, type: isConflict ? 'error' : 'success' },
          {
            text: isConflict
        ...
        if (localVersion !== remoteVersion) {
          ...
          setOutputLog((prev) => [
            ...prev,
            { text: '$ qikenv push', type: 'info' },
            { text: `Optimistic concurrency check passed: Remote SHA [${localHash}] == Local SHA.`, type: 'success' },
            { text: `Encrypted payload transmitted to vault. Remote advanced to v1.${nextVer}.0 [${nextHash}].`, type: 'success' }
          ]);
        }
      } else if (cmd === 'pull') {
        ...
        setOutputLog((prev) => [
          ...
          { text: `Synchronized local workspace with remote v1.${remoteVersion}.0 [${remoteHash}].`, type: 'success' },
        ...
        setOutputLog((prev) => [
          ...
          { text: 'Executing 3-way hash reconciliation without echoing secret values...', type: 'info' },
          { text: 'Auto-merged: +REDIS_CACHE_TTL (remote), +STRIPE_WEBHOOK_SECRET (local)', type: 'info' },
          { text: `Reconciliation verified. New version sealed: v1.${nextVer}.0 [${nextHash}].`, type: 'success' }
        ]);
      }
  ...
  return (
    <div className="border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between border-b border-[var(--border)] bg-[var(--surface-alt)] px-3 sm:px-4 py-2.5 gap-2">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-mono border ${
              isDiverged
          ...
          <button
            onClick={resetDemo}
            className="inline-flex items-center gap-1 border border-[var(--border)] bg-[var(--surface)] px-2 py-1 text-[11px] font-mono text-[var(--text-muted)] hover:text-[var(--text-main)] hover:border-[var(--text-main)] transition-colors cursor-pointer"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 border-b border-[var(--border)] divide-y sm:divide-y-0 sm:divide-x divide-[var(--border)] bg-[var(--surface)] text-xs">
        ...
        <div className="p-3 sm:p-4">
          <div className="flex items-center justify-between text-[11px] font-mono text-[var(--text-faint)]">
            ...
            <span className="text-[var(--text-main)] font-semibold">Workspace</span>
          </div>
          <div className="mt-1 flex items-baseline gap-2 font-mono">
            <span className="text-sm font-semibold text-[var(--text-main)]">v1.{localVersion}.0</span>
            <span className="text-[11px] text-[var(--text-muted)]">SHA: {localHash}</span>
          </div>
          <p className="mt-1 text-[11px] text-[var(--text-muted)]">
            Base revision anchored when developer initialized or pulled repository.
          </p>
        </div>
        ...
        <div className="p-3 sm:p-4">
          <div className="flex items-center justify-between text-[11px] font-mono text-[var(--text-faint)]">
            ...
            <span className="text-[var(--text-main)] font-semibold">PostgreSQL</span>
          </div>
          <div className="mt-1 flex items-baseline gap-2 font-mono">
            <span className="text-sm font-semibold text-[var(--text-main)]">v1.{remoteVersion}.0</span>
            <span className="text-[11px] text-[var(--text-muted)]">SHA: {remoteHash}</span>
          </div>
          <p className="mt-1 text-[11px] text-[var(--text-muted)]">
            Canonical source of truth verified prior to committing changes to storage.
          </p>
        </div>
      </div>
      ...
      <div className="border-b border-[var(--border)] bg-[var(--surface-alt)] p-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-mono text-[var(--text-faint)] mr-1 hidden sm:inline">RUN CLI:</span>

          <button
            onClick={() => handleCommand('status')}
            disabled={isProcessing}
            className="min-h-[38px] sm:min-h-0 border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs font-mono text-[var(--text-main)] hover:border-[var(--text-main)] transition-colors disabled:opacity-50 cursor-pointer"
          >
            qikenv status
          </button>
          ...
          {isDiverged && (
            <button
              ...
              disabled={isProcessing}
              className="min-h-[38px] sm:min-h-0 border border-[var(--accent)] bg-[var(--accent-subtle)] px-3 py-1.5 text-xs font-mono text-[var(--accent)] font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer"
            >
          ...
          <div className="sm:ml-auto w-full sm:w-auto mt-1 sm:mt-0">
            <button
              ...
              className={`w-full sm:w-auto min-h-[38px] sm:min-h-0 inline-flex items-center justify-center gap-1.5 border px-3 py-1.5 text-xs font-mono transition-colors ${
                teammatePushed
            ...
            >
              ...
              <span>{teammatePushed ? 'Teammate Diverged' : 'Simulate Teammate Push'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#09090b] text-[#f4f4f5] p-3 sm:p-4 font-mono text-xs max-h-56 overflow-y-auto space-y-1">
      ...
      <div className="border-t border-[var(--border)] bg-[var(--surface-alt)] px-3 sm:px-4 py-2 text-[11px] text-[var(--text-muted)] font-mono flex flex-wrap items-center justify-between gap-2">
        <span>OCC Principle: Hash revisions prevent race conditions without table locks.</span>
        <a
